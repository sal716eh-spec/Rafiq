...
**Practise** holds the extra practice: **real-life scenes** (six conversations — airport, doctor, masjid, restaurant, taxi, visiting a family — in `scenes-data.js`, played with the conversation partner; Complete), a **weak-spots review** (`session.html?focus=weak`: exercises tagged with the learner's repeat mistake types in `item-tags.js`, generated by `tools/typesafe-exp/tag_items.py`, plus items they recently missed; Complete), Words (spaced repetition over all 779 words), Sentences (13 drill modes across 12 units), Verbs, Joining words, and a mixed review.
